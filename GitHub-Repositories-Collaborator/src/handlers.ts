import {exceptions, ProgressEvent,} from '@amazon-web-services-cloudformation/cloudformation-cli-typescript-lib';
...
class Resource extends AbstractGitHubResource<ResourceModel, GetCollaboratorPayload, PutCollaboratorPayload, PutCollaboratorPayload, TypeConfigurationModel> {

    private userAgent = `AWS CloudFormation (+https://aws.amazon.com/cloudformation/) CloudFormation resource ${this.typeName}/${version}`;

    async get(model: ResourceModel, typeConfiguration?: TypeConfigurationModel): Promise<GetCollaboratorPayload> {
    ...
    async list(model: ResourceModel, typeConfiguration?: TypeConfigurationModel): Promise<ResourceModel[]> {
        ...
        const collaborators = await octokit.paginate(
            ...
            response => response.data.map(membershipItem => {
                ...
                resourceModel.permission = Resource.getPermissionName(membershipItem.permissions);
                return resourceModel;
            }));
        const invitations = await this.listInvitationsByRepo(model, typeConfiguration);

        return collaborators.concat(invitations.map(invitation => {
            ...
            invitationAsResourceModel.permission = this.permissionsToPermission(invitation.permissions);
            return invitationAsResourceModel;
        }));
    }

    async create(model: ResourceModel, typeConfiguration?: TypeConfigurationModel): Promise<PutCollaboratorPayload> {
        if (await this.assertIsCollaborator(model, typeConfiguration) || await this.assertHasInvitationPending(model, typeConfiguration)) {
            throw {
    ...
    async update(model: ResourceModel, typeConfiguration?: TypeConfigurationModel): Promise<PutCollaboratorPayload> {
        if (await this.assertIsCollaborator(model, typeConfiguration)) {
            const response = await this.putCollaborator(model, typeConfiguration);
            return response.data;
        }
        if (await this.assertHasInvitationPending(model, typeConfiguration)) {
            const response = await this.updateRepoInvitation(model, typeConfiguration);
            return response.data;
        }
    ...
    setModelFrom(model: ResourceModel, from?: CollaboratorData): ResourceModel {
        return new ResourceModel(!from
            ? model
            : {
                ...model,
                invitationId: from.id,
                owner: from.repository.owner.login,
                repository: from.repository.name,
                username: from.invitee.login,
                permission: this.permissionsToPermission(from.permissions)
            });
    }
    ...
    private async updateRepoInvitation(model: ResourceModel, typeConfiguration: TypeConfigurationModel) {
    ...
    private async listInvitationsByRepo(model: ResourceModel, typeConfiguration?: TypeConfigurationModel) {
        const octokit = new Octokit({
            auth: typeConfiguration?.gitHubAccess.accessToken,
            userAgent: this.userAgent
        });
    ...
    private async assertIsCollaborator(model: ResourceModel, typeConfiguration?: TypeConfigurationModel): Promise<boolean> {
        try {
            const collaborator = await this.get(model, typeConfiguration);
    ...
    private async assertHasInvitationPending(model: ResourceModel, typeConfiguration?: TypeConfigurationModel) {
        const invitations = await this.listInvitationsByRepo(model, typeConfiguration);
...
//
// class Resource2 extends BaseResource<ResourceModel> {
//
//     private userAgent = `AWS CloudFormation (+https://aws.amazon.com/cloudformation/) CloudFormation resource ${this.typeName}/${version}`;
//
...
//         model.permission = Resource2.permissionsToPermission(data.permissions);
//         return model;
//     }
//
...
//     }
//
//     private async getCollaborator(model: ResourceModel, request: ResourceHandlerRequest<ResourceModel>): Promise<OctokitResponse<GetCollaboratorPayload>> {
//         const octokit = new Octokit({
//             auth: model.gitHubAccess,
//             userAgent: this.userAgent
//         });
...
//      * @private
//      */
//     private async createOrUpdateCollaborator(model: ResourceModel, request: ResourceHandlerRequest<ResourceModel>): Promise<OctokitResponse<PutCollaboratorPayload>> {
//         const octokit = new Octokit({
//             auth: model.gitHubAccess,
//             userAgent: this.userAgent
//         });
